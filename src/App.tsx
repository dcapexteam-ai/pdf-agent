...
export default function App() {
  ...
  async function deliver(items: Array<{ name: string, blob: Blob }>) {
    ...
    if (outputTarget === 'email') {
      ...
      // If multiple items, zip them in-memory
      let payloads = items
      if (items.length > 1) {
...
async function zipBlobs(items: Array<{ name: string, blob: Blob }>): Promise<Blob> {
  // Simple zip via CompressionStream if available; otherwise fallback to single-attachment behavior
  // Note: CompressionStream only supports gzip/deflate, not zip container. We'll implement a minimal ZIP with JSZip? But that adds dep.
  // To keep deps minimal, we include JSZip.
